...
import { useCollegeSupervisors, useUpdateUserStatus } from '../../hooks/useUsers';
...
const SupervisorsManagement = () => {
    const { data: apiResponse, isLoading, isError, refetch } = useCollegeSupervisors();
    ...
    const supervisors = Array.isArray(apiResponse) ? apiResponse : (apiResponse?.data || []);
    const departments = Array.isArray(departmentsResponse) ? departmentsResponse : (departmentsResponse?.data || []);

    const filteredData = supervisors.filter(supervisor => {
        const matchesSearch = supervisor.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) || 
                             supervisor.email?.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesDepartment = selectedDepartment === 'جميع الأقسام' || supervisor.departmentName === selectedDepartment;
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-500">
                ...
                <p>حدث خطأ أثناء تحميل بيانات المشرفين</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-6 w-full">
            <nav className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                ...
                <span className="text-slate-900 dark:text-white font-medium">إدارة المشرفين</span>
            </nav>

            <div className="max-w-7xl mx-auto w-full flex flex-col gap-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex flex-col gap-1">
                        <h1 className="text-3xl font-black leading-tight tracking-tight text-slate-900 dark:text-white">إدارة المشرفين</h1>
                        <p className="text-slate-500 dark:text-slate-400 text-base">بوابة إدارية لمتابعة أعضاء هيئة التدريس وإدارة حساباتهم.</p>
                    </div>
                    <button 
                        ...
                        <table className="w-full text-right border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">الاسم</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">القسم</th>
                                    <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">البريد الإلكتروني</th>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {filteredData.map((supervisor) => (
                                    <tr key={supervisor.userID} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <p className="text-sm font-semibold text-slate-900 dark:text-white">{supervisor.fullName}</p>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{supervisor.departmentName || 'غير محدد'}</td>
                                        <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{supervisor.email}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-1.5">
                                                <div className={`size-1.5 rounded-full ${supervisor.isActive ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`}></div>
                                                <span className={`text-xs font-medium ${supervisor.isActive ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>
                                        ...
                                        <td className="px-6 py-4 text-left">
                                            <ActionMenu 
                                                isOpen={activeActionRowId === supervisor.userID}
                                                onToggle={() => setActiveActionRowId(activeActionRowId === supervisor.userID ? null : supervisor.userID)}
                                                onClose={() => setActiveActionRowId(null)}
                                                isActive={supervisor.isActive}
                                                onActivate={() => updateStatusMutation.mutate({ id: supervisor.userID, isActive: true })}
                                                onDeactivate={() => updateStatusMutation.mutate({ id: supervisor.userID, isActive: false })}
                                                onEdit={() => {
                                                    setSelectedUserId(supervisor.userID);
