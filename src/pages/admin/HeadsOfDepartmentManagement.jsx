...
import { useCollegeHODs, useUpdateUserStatus } from '../../hooks/useUsers';
...
const HeadsOfDepartmentManagement = () => {
    const { data: apiResponse, isLoading, isError, refetch } = useCollegeHODs();
    ...
    const heads = Array.isArray(apiResponse) ? apiResponse : (apiResponse?.data || []);
    ...
    const filteredData = heads.filter(head => {
        ...
        const matchesDepartment = selectedDepartment === 'جميع الأقسام' || head.departmentName === selectedDepartment;
    ...
    if (isError) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-500">
                ...
                <p>حدث خطأ أثناء تحميل بيانات رؤساء الأقسام</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-6 w-full">
            <nav className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                ...
                <span className="text-slate-900 dark:text-white font-medium">إدارة رؤساء الأقسام</span>
            </nav>

            <div className="max-w-7xl mx-auto w-full flex flex-col gap-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex flex-col gap-1">
                        <h1 className="text-3xl font-black leading-tight tracking-tight text-slate-900 dark:text-white">إدارة رؤساء الأقسام</h1>
                        <p className="text-slate-500 dark:text-slate-400 text-base">بوابة إدارية لمتابعة رؤساء الأقسام الأكاديمية وإدارة حساباتهم.</p>
                    </div>
                    <button 
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {filteredData.map((head) => (
                                    <tr key={head.userID} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{head.departmentName || 'غير محدد'}</td>
                                        <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{head.email}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-1.5">
                                                <div className={`size-1.5 rounded-full ${head.isActive ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`}></div>
                                                <span className={`text-xs font-medium ${head.isActive ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>
                                                    {head.isActive ? 'نشط' : 'غير نشط'}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-left">
                                            <ActionMenu 
                                                ...
                                                onActivate={() => updateStatusMutation.mutate({ id: head.userID, isActive: true })}
                                                onDeactivate={() => updateStatusMutation.mutate({ id: head.userID, isActive: false })}
                                                onEdit={() => {
                                                    setSelectedUserId(head.userID);
...
export default HeadsOfDepartmentManagement;
